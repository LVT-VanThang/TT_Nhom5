...
<body>
  <div class="container">
    <aside class="sidebar">
      ...
      <ul>
        <li><a href="${pageContext.request.contextPath}/view/trangchuadmin.jsp">🏠 Trang chủ</a></li>
    ...
    <form action="${pageContext.request.contextPath}/ThuThu" method="get" style="display: flex; gap: 10px; flex: 1;">
        <input type="text" name="tuKhoa" value="${tuKhoa}" placeholder="Tìm kiếm theo tên hoặc tên đăng nhập..." style="flex: 1;" />
        <button type="submit" class="btn-search">🔍 Tìm kiếm</button>
        <c:if test="${not empty tuKhoa}">
            <a href="${pageContext.request.contextPath}/ThuThu" class="btn-delete" style="text-decoration: none; display: flex; align-items: center;">Hủy tìm</a>
        </c:if>
    </form>
    <a href="${pageContext.request.contextPath}/ThuThu?action=new" class="btn-add" style="text-decoration: none; display: inline-block;">
    ➕ Thêm mới
    </a>
</div>
          ...
          <c:if test="${not empty dstt}">
              <c:forEach var="tt" items="${dstt}" varStatus="status">
                  <tr>
                    ...
                    <td>
                      <a href="ThuThu?action=edit&id=${tt.maThuThu}" class="btn-edit">Sửa</a>
   ...
   Xóa
          ...
          <c:if test="${empty dstt}">
              <tr>
                  <td colspan="7" style="text-align:center;">Không có dữ liệu thủ thư nào.</td>
              </tr>
          </c:if>
        </tbody>
      </table>
  ...
  <div id="modalThem" class="modal">
    <div class="modal-content">
      ...
      <form action="${pageContext.request.contextPath}/ThuThu" method="post">
          
          <input type="hidden" name="action" value="${not empty suaThuThu ? 'update' : 'insert'}">
          ...
          <div class="form-group">
              ...
              <input type="text" name="matKhau" 
                     value="${not empty suaThuThu ? suaThuThu.matKhau : ''}" 
                     required>
          </div>
          ...
          <div class="form-group">
              ...
              <select name="trangThai">
                  <option value="1" ${suaThuThu.trangThai == 1 ? 'selected' : ''}>Hoạt động</option>
                  <option value="0" ${suaThuThu.trangThai == 0 ? 'selected' : ''}>Đã khóa</option>
              </select>
          </div>
  ...
  <script src="${pageContext.request.contextPath}/view/script.js?v=3"></script>
  
  <c:if test="${not empty baoLoi or not empty suaThuThu or not empty moFormThem}">
      <script>
