...
// --- PHẦN 7: HÀM VẼ BIỂU ĐỒ (CHART.JS) ---
function veBieuDoTopSach(labels, data) {
    var canvas = document.getElementById('topBookChart');
    
    // Kiểm tra nếu không có thẻ canvas thì dừng
    if (!canvas) return;
    ...
    new Chart(ctx, {
        type: 'bar', 
        data: {
            labels: labels, // Dữ liệu tên sách từ tham số
            datasets: [{
                label: 'Số lượt mượn',
                data: data, // Dữ liệu số lượt từ tham số
                backgroundColor: [
        ...
        options: {
            responsive: true,
            maintainAspectRatio: false, // Giữ chiều cao cố định
            scales: {
                y: {
                    beginAtZero: true,
