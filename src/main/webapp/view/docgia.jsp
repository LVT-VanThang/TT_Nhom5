...
<body>
  <div class="container">
    <aside class="sidebar">
      ...
      <ul>
        ...
        <li><a href="${pageContext.request.contextPath}/DocGia" class="active">🧑‍💼 Độc giả</a></li>
          ...
          <c:if test="${not empty dsdg}">
            <c:forEach var="dg" items="${dsdg}" varStatus="status">
              <tr>
                ...
                <td>
                    <span style="color: ${dg.trangThaiThe == 'Hoạt động' ? 'green' : 'red'}; font-weight:bold;">
                        ${dg.trangThaiThe}
                    </span>
                </td>
                <td>
  ...
  <div id="modalThem" class="modal">
    <div class="modal-content">
      ...
      <h2>
        <c:choose>
          <c:when test="${not empty suaDocGia}">✏️ Cập nhật Độc giả</c:when>
          <c:otherwise>➕ Thêm mới Độc giả</c:otherwise>
        </c:choose>
      </h2>

      <form action="${pageContext.request.contextPath}/DocGia" method="post">
        <input type="hidden" name="action" value="${not empty suaDocGia ? 'update' : 'insert'}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="maDocGia"
                ...
                ${not empty suaDocGia ? 'readonly style="background-color:#e9ecef"' : ''}
        ...
        <div class="form-group">
            ...
            <select name="trangThaiThe" required>
             ...
             <option value="Hết Hạn">Đã khóa (Hết Hạn)</option>
         </select>
        </div>

        <button type="submit" class="btn-save" style="margin-top: 20px;">
          <c:choose>
            <c:when test="${not empty suaDocGia}">💾 Cập nhật</c:when>
  ...
  <script src="${pageContext.request.contextPath}/view/script.js?v=10"></script>
