...
<body>
	<div class="container">
		...
		<main class="main-content">
			...
			<div class="search-bar">
				<form action="${pageContext.request.contextPath}/NhaXuatBan"
					...
					<c:if test="${not empty param.tuKhoa}">
						<a href="${pageContext.request.contextPath}/NhaXuatBan"
							class="btn-delete"
						...
						<c:forEach var="nxb" items="${dsnxb}" varStatus="status">
							<tr>
								...
								<td><a href="NhaXuatBan?action=edit&id=${nxb.maNXB}"
									...
									onclick="xacNhanXoa('${nxb.maNXB}', '${nxb.tenNXB}', 'NhaXuatBan')">
										Xóa </a></td>
							</tr>
						</c:forEach>
					</c:if>

					<c:if test="${empty dsnxb}">
						<tr>
	...
	<div id="modalThem" class="modal">
		<div class="modal-content">
			...
			<h2>
				<c:choose>
					<c:when test="${not empty suaNhaXuatBan}">✏️ Cập nhật Thể Loại</c:when>
			...
			<form action="${pageContext.request.contextPath}/NhaXuatBan"
				...
				<input type="hidden" name="action"
					value="${not empty suaNhaXuatBan ? 'update' : 'insert'}">
				...
				<div class="form-group">
					<label>Mã Thể Loại (*):</label> <input type="text" name="maNhaXuatBan"
						value="${not empty suaNhaXuatBan ? suaNhaXuatBan.maNXB : param.maNhaXuatBan}"
						${not empty suaNhaXuatBan ? 'readonly style="background-color:#e9ecef"' : ''}
						required placeholder="Nhập mã (VD: NXB001)">
				</div>

				<div class="form-group">
					<label>Tên Quy định (*):</label> <input type="text"
						name="tenNhaXuatBan"
						value="${not empty suaNhaXuatBan ? suaNhaXuatBan.tenNXB : param.tenNhaXuatBan}"
				...
				<button type="submit" class="btn-save" style="margin-top: 20px;">
					<c:choose>
						<c:when test="${not empty suaNhaXuatBan}">💾 Cập nhật</c:when>
