...
<head>
  ...
  <link rel="stylesheet" href="${pageContext.request.contextPath}/view/style.css?v=1" />
...
<body>
  <div class="container">
    ...
    <main class="main-content">
      ...
      <div class="search-bar">
        <form action="${pageContext.request.contextPath}/QuyDinh" method="get" style="display: flex; gap: 10px; flex: 1;">
            <input type="text" name="tuKhoa" value="${param.tuKhoa}" placeholder="Tìm kiếm theo tên quy định..." style="flex: 1;" />
            <button type="submit" class="btn-search">🔍 Tìm kiếm</button>
            <c:if test="${not empty param.tuKhoa}">
                <a href="${pageContext.request.contextPath}/QuyDinh" class="btn-delete" style="text-decoration: none; display: flex; align-items: center;">Hủy tìm</a>
            </c:if>
        </form>
          ...
          <c:if test="${not empty dsqd}">
              <c:forEach var="qd" items="${dsqd}" varStatus="status">
                  <tr>
                    ...
                    <td>${qd.giaTri}</td>
                    <td>${qd.donViTinh}</td>
                    <td><fmt:formatDate value="${qd.ngayCapNhat}" pattern="dd/MM/yyyy HH:mm"/></td>
                    <td>${qd.quanTriVien.maAdmin}</td>
                    <td>
                      <a href="QuyDinh?action=edit&id=${qd.maQuyDinh}" class="btn-edit">Sửa</a>
   ...
   Xóa
          ...
          <c:if test="${empty dsqd}">
              <tr>
                  <td colspan="8" style="text-align:center;">Không có dữ liệu quy định nào.</td>
              </tr>
          </c:if>
        </tbody>
      </table>
  ...
  <div id="modalThemQuyDinh" class="modal">
    <div class="modal-content">
      ...
      <form action="${pageContext.request.contextPath}/QuyDinh" method="post">
          
          <input type="hidden" name="action" value="${not empty suaQuyDinh ? 'update' : 'insert'}">
          ...
          <div class="form-group">
              ...
              <input type="text" name="maQuyDinh" 
          ...
          <div class="form-group">
              ...
              <input type="text" name="tenQuyDinh" 
  ...
  <c:if test="${not empty baoLoi or not empty suaQuyDinh or not empty moFormThem}">
      <script>
          document.addEventListener("DOMContentLoaded", function() {
              var modal = document.getElementById("modalThemQuyDinh");
              if(modal) {
